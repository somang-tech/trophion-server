...
## 트로피 이미지 3차 수정 (로고와 통일 / 1080x1080 / 업적 정렬)

- **트로피 모양을 사이트 로고와 동일하게**: 직전 라운드에서 만든 화려한 벌브형 트로피가
  실제 헤더 로고와 너무 달라 보인다는 피드백을 받고, 로고(`.brand-mark`)와 **정확히 같은
  패스 데이터**(볼+손잡이 2개+기둥+받침대, `M8 6H24V13C…`)를 그대로 확대해서 쓰도록 다시
  그렸다. 로고는 얇은 선 아이콘이라 큰 사이즈에서 허전해 보이는 것만 볼 안쪽 그라디언트
  채움 + 작은 별 포인트로 보완했다. `public/index.html`의 `bigTrophySvg()`와
  `src/shareCard.js`의 `drawLogoTrophy()`가 완전히 같은 좌표계(viewBox/캔버스 0~32)를 써서
  화면과 다운로드 이미지가 같은 모양으로 나온다.
- **다운로드 이미지에서 텍스트가 깨지거나 안 보이던 문제**: 트로피 이름 등 한글 텍스트가
  DejaVu Sans 폰트에 없는 한글 글리프라 네모(tofu)로 깨져 보였다. 근본 원인은 업적이 아직
  한국어로 캐시돼 있었기 때문 — 아래 "언어" 항목대로 재동기화하면 텍스트가 영어로 바뀌어서
  이 문제 자체가 사라진다. (한글이 하나도 안 남게 되므로 별도로 한글 폰트를 번들하진 않았다
  — CJK 폰트는 용량이 20MB+라 리포가 너무 무거워진다.)
- **상단 TROPHION 로고/워드마크를 사이트 헤더와 동일하게**: 다운로드 이미지의 브랜드마크가
  아이콘 모양도, 글꼴도, 색도 실제 헤더와 달랐던 문제. 아이콘은 헤더와 완전히 같은
  패스로 다시 그렸고(`drawBrandMark()`), 워드마크는 헤더가 쓰는 구글폰트 **Bebas Neue**를
  이 샌드박스에서 네트워크가 막혀 있어 받아오지 못해서, 같은 계열의 콘덴스드 디스플레이
  서체(Big Shoulders Bold, `fonts/BigShoulders-Bold.ttf`)로 대체하고 흰색→골드 그라디언트
  + 글자 간격까지 맞춰서 최대한 비슷하게 만들었다. 실제 Bebas Neue를 쓰고 싶으면
  `fonts/`에 `.ttf`를 넣고 `src/shareCard.js`의 `FONT_DISPLAY` 등록 경로만 그 파일로
  바꾸면 된다.
- **다운로드 이미지 크기를 1080×1080(인스타/페이스북 정사각형 표준)으로 고정**:
  `src/shareCard.js`의 `W`, `H` 상수를 1200×630(OG 비율)에서 1080×1080으로 바꾸고, 트로피
  크기(골드 400px 등)와 텍스트도 그 안에서 꽉 차게 다시 배치했다. **화면 미리보기(대시보드의
  `.tpl-card`)는 그대로 와이드 카드 레이아웃**이고, 다운로드할 때만 이 정사각형으로 렌더링된다
  — 요청하신 그대로다.
- **업적 정렬 순서**: 화면의 업적 그리드와 "획득한 업적" 랙 둘 다 (1) 깬 업적을 위로, 안 깬
  업적을 아래로, (2) 그 안에서는 등급 높은 순(Legendary → Epic → Rare → Common)으로
  정렬하도록 `sortForDisplay()`를 추가해서 `renderAchievements()`/`renderRack()`에 적용했다.

## 트로피 이미지 4차 수정 — 진짜 원인은 "덩어리처럼 보이던" 렌더링 버그였다

3차 수정에서 로고 패스를 그대로 키워 썼는데도 여전히 트로피가 아니라 이상한 두 쪽짜리
덩어리처럼 보인 이유는 두 가지 버그가 겹쳤기 때문이다:

1. **stroke-width 이중 스케일링(화면 쪽)**: `bigTrophySvg()`가 `stroke-width`를
   `size/32`로 미리 곱해놓은 채로 넘겼는데, SVG의 `viewBox`가 원래 알아서 그 비율만큼
   또 한 번 늘려준다 — 결과적으로 선 두께가 제곱으로 두꺼워져서(예: 228px 트로피에서
   실제 선 두께가 100px 가까이) 선이 아니라 덩어리가 됐다. 이제 `stroke-width`는 로고
   원본 값(2 정도)을 그대로 쓰고, 크기 조절은 `viewBox` 스케일링에만 맡긴다.
2. **볼(컵 몸통)을 꽉 채운 것(서버 쪽)**: 로고의 컵 바닥은 두 베지어 곡선이 한 점(16,22)에서
   뾰족하게 만나는 모양이다. 얇은 선 아이콘일 땐 안 보이던 이 뾰족점이, 안쪽을 진하게
   채우자 "두 봉우리가 가운데서 만나는 덩어리"로 도드라져 보였다. 이제 볼 안쪽은 12%
   정도만 아주 옅게 채우고 나머지는 로고와 똑같이 "선" 위주로 그린다.

이 두 버그를 고치고 나니 로고와 완전히 같은 실루엣의 심플한 아이콘이 됐고, 여기에 요청하신
**화려한 이펙트**로 뒤쪽 은은한 후광(glow, 그라디언트 원)과 아이콘 주변에 반짝이는 별 4~5개
(sparkle)를 추가했다. `public/index.html`의 `bigTrophySvg()`와 `src/shareCard.js`의
`drawLogoTrophy()`가 같은 로직(글로우+반짝이+로고 패스)으로 화면/다운로드 이미지를 동일하게
그린다.

**배경 이미지 크롭 문제**: 정사각형(1080×1080) 캔버스에 보통 16:9 비율인 게임 커버를
"꽉 채우기(cover)"로 넣으면 좌우나 위아래가 크게 잘려나간다. `src/shareCard.js`에서
`Math.max` → `Math.min`으로 바꿔서(꽉 채우기 → 안에 맞추기/contain) 이미지 전체가 잘리지
않고 다 보이도록 했고, 남는 여백은 기존 배경 그라디언트가 자연스럽게 채운다.

**우측 상단 로고에 대해 솔직히**: 아이콘(트로피 모양)은 헤더와 완전히 같은 패스로 그려서
동일하다. 다만 "TROPHION" 글자 폰트는 사이트가 쓰는 구글폰트 Bebas Neue를 이 작업 환경의
네트워크 제한 때문에 받아올 수 없어서 Big Shoulders Bold로 대체한 상태 그대로다 — 자간을
더 좁혀서(letter-spacing 2px→0.5px) 최대한 Bebas Neue의 촘촘한 느낌에 가깝게 만들었지만,
100% 같은 폰트는 아니다. 완전히 똑같이 만들려면 실제 Bebas Neue `.ttf` 파일이 필요한데,
이건 사용자분 쪽에서 (a) Google Fonts에서 직접 받아서 `fonts/BebasNeue-Regular.ttf`로 넣고
`src/shareCard.js`의 `FONT_DISPLAY` 등록 줄만 그 파일로 바꿔주시거나, (b) 다음에 네트워크
제약이 없는 환경에서 다시 요청해주시면 제가 받아서 넣어드릴 수 있다.

## 트로피 이미지 5차 수정 — "다운로드가 미리보기랑 다르다" 문제를 구조적으로 해결

3~4차 수정에서 서버가 매번 트로피/로고/배경을 처음부터 다시 그리는 방식(`shareCard.js`)을
계속 화면과 맞추려고 했는데, 그때그때 미세하게 어긋나는 문제가 반복됐다(선 굵기, 폰트,
배경 크롭 등). 근본 원인은 "화면에 보이는 것"과 "서버가 다시 그리는 것"이 애초에 다른
두 개의 구현이었기 때문이다.

그래서 **"이미지 다운로드" 버튼의 방식을 통째로 바꿨다**: 서버에서 다시 그리는 대신,
[html2canvas](https://html2canvas.hertzen.com/)로 **화면에 지금 보이는 트로피 카드
(`#tplCard`)를 그대로 캡처**해서 PNG로 내려받는다. 이러면 트로피 모양이든 TROPHION
로고 폰트든 배경 이미지 처리든 화면과 100% 같을 수밖에 없다 — 애초에 같은 픽셀이니까.

- `public/index.html`에 html2canvas를 CDN(`cdnjs.cloudflare.com`)으로 로드했다.
- 다운로드 버튼 클릭 시: (1) 버튼 자체가 캡처에 찍히지 않게 잠깐 숨기고, (2) 웹폰트
  로딩이 끝나길 기다린 뒤(`document.fonts.ready`), (3) `html2canvas(card, {scale: 2~3})`로
  고해상도 캡처, (4) `canvas.toBlob()` → `<a download>`로 저장한다.
- **이미지 크기**: 요청하신 대로 1080×1080을 강제하지 않는다. 캡처 결과는 화면에 실제로
  렌더링된 카드의 크기(= 배경 사진 비율에 맞춰 이미 레이아웃된 실제 크기)를 그대로
  따라가고, `scale` 옵션으로 해상도만 2~3배 키운다.
- **CORS 이슈 방지 겸 커버 이미지 문제 통합 해결**: `html2canvas`가 크로스 오리진
  이미지(Steam CDN)를 캡처하면 캔버스가 "오염(tainted)"되어 다운로드 자체가 막힐 수 있다.
  그래서 새 라우트 `GET /img/cover/:appId`(`src/index.js`)를 추가해 우리 서버가 Steam
  CDN 이미지를 대신 받아와 같은 출처(same-origin)로 내려주도록 했다 — 이 프록시가
  `header.jpg → capsule_616x353.jpg → …` 후보들을 내부적으로 순서대로 시도해주기 때문에,
  기존에 프론트에 있던 다중 URL 재시도 로직(`onCoverError`의 후보 배열)도 이번에 훨씬
  단순해졌다(`coverImgAttrs()`가 이제 이 프록시 URL 하나만 준다).
- `src/shareCard.js`(서버 사이드 PNG 렌더러)는 이제 **디스코드/트위터 링크 미리보기용
  OG 이미지**(`GET /u/:steamId64/:appId.png`, JS를 실행할 수 없는 상황이라 여기만
  서버 렌더링이 꼭 필요함) 전용으로만 쓰인다. 정사각형(1080×1080)에서 OG 표준 비율
  (1200×630)로 되돌렸고, 별 포인트를 빼고 후광을 화면 쪽과 같은 4단 그라디언트로 맞췄다.

## 트로피 비주얼 다듬기 (별 제거 / 후광 자연스럽게)

- 트로피 안의 작은 별 포인트를 없앴다("너무 투박하다"는 피드백) — 지금은 컵+손잡이+기둥+
  받침대만 있는, 로고와 완전히 같은 심플한 실루엣이다.
- 후광(glow)을 단일 원 그라디언트에서 **4단 그라디언트**(중심 밝은 색 → 메인 색 → 옅게 →
  투명)로 바꿔서 경계가 뚝 끊기지 않고 자연스럽게 번지도록 했다. 반짝이(sparkle) 4~5개는
  그대로 유지.

## 언어 — 아직도 한글로 나온다면

지난 라운드에서 업적 텍스트를 영어로 고정하도록 고쳤지만(`steamLang = 'english'`), 이건
**다음 동기화부터** 적용된다 — DB에 이미 캐시된 업적 이름/설명은 예전에 한국어로 받아온
그대로 남아있다. 화면 상단의 "새로고침"(Sync) 버튼을 한 번 더 눌러서 재동기화하면 그때
